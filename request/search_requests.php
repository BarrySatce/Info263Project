...
$customerQuery =  "SELECT concat(owner_fname, ' ', owner_lname) AS 'NAME', owner_email AS 'EMAIL', owner_phone AS 'PHONE',
...
$alignmentQuery =  "SELECT rear_axle_camber_left_before AS '1', 
...
if (isset($_POST['input'])){
    ...
    //Setting up query searches
    $customerQuery .= "WHERE invoice_id LIKE '%" .$search_term."%' ";
    $invoiceQuery .= "WHERE invoice_id LIKE '%" .$search_term."%' ";
    $branchQuery .= "WHERE invoice_id LIKE '%" .$search_term."%' ";
    $alignmentQuery .= "WHERE invoice_id LIKE '%" .$search_term."%' ";

    //get results
    $customerQueryResponse = mysqli_query($conn, $customerQuery) or die(mysqli_error($conn));
    $invoiceQueryResponse = mysqli_query($conn, $invoiceQuery) or die(mysqli_error($conn));
    $branchQueryResponse = mysqli_query($conn, $branchQuery) or die(mysqli_error($conn));
    $alignmentQueryResponse = mysqli_query($conn, $alignmentQuery) or die(mysqli_error($conn));

    //get rows
    $customerRow = mysqli_fetch_array($customerQueryResponse);
    $invoiceRow = mysqli_fetch_array($invoiceQueryResponse);
    $branchRow = mysqli_fetch_array($branchQueryResponse);
    $alignmentRow = mysqli_fetch_array($alignmentQueryResponse);
}
